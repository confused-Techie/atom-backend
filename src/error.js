...
function SiteWide404(res) {
  res.status(404).json({ message: "This is a standin for the proper site wide 404 page."});
}

function MissingAuthJSON(res) {
  res.status(401).json({ message: "Requires authentication. Please update your token if you haven't done so recently."});
}
