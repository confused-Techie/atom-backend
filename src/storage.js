...
async function getBanList() {
  ...
  const getNew = async function () {
    try {
      ...
      let contents = await gcs_storage.bucket(GCLOUD_STORAGE_BUCKET).file("name_ban_list.json").download();
...
async function getFeaturedPackages() {
  ...
  const getNew = async function () {
    try {
      ...
      let contents = await gcs_storage.bucket(GCLOUD_STORAGE_BUCKET).file("featured_packages.json").download();
